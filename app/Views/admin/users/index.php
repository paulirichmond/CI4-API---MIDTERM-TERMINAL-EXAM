                ...
                <tbody style="border-top: none;">
                    ...
                    <?php
                        ...
                        if ($u['role_name'] === 'admin') {
                            $badgeBg = 'rgba(239,68,68,0.14)'; $badgeColor = '#ef4444'; $badgeBorder = 'rgba(239,68,68,0.2)'; $roleDotColor = '#ef4444';
                        } elseif ($u['role_name'] === 'teacher') {
                            $badgeBg = 'rgba(16,185,129,0.14)'; $badgeColor = '#10b981'; $badgeBorder = 'rgba(16,185,129,0.2)'; $roleDotColor = '#10b981';
                        } elseif ($u['role_name'] === 'student') {
                            $badgeBg = 'rgba(59,130,246,0.14)'; $badgeColor = '#3b82f6'; $badgeBorder = 'rgba(59,130,246,0.2)'; $roleDotColor = '#3b82f6';
                        } elseif ($u['role_name'] === 'coordinator') {
                            $badgeBg = 'rgba(245,158,11,0.14)'; $badgeColor = '#f59e0b'; $badgeBorder = 'rgba(245,158,11,0.2)'; $roleDotColor = '#f59e0b';
                        }
                    ?>
                        ...
                        <td class="py-3 pe-4 text-end" style="border-bottom: 1px solid var(--border-light);">
                            ...
                            <form action="<?= base_url('/admin/users/assign-role/' . $u['id']) ?>" method="POST" class="d-flex align-items-center justify-content-end gap-2 m-0">
                                ...
                                <span style="display:inline-flex;align-items:center;gap:8px;">
                                    <span class="role-indicator" title="<?= esc($u['role_label'] ?? 'Role') ?>" style="width:12px;height:12px;border-radius:50%;background: <?= $roleDotColor ?>;display:inline-block;box-shadow:0 0 0 3px rgba(0,0,0,0.28);"></span>
                                    <select name="role_id" class="clean-input py-1 px-2 m-0" style="font-size: 0.8rem; height: 32px; width: auto; max-width: 150px; background: #0f172a; color: #ffffff; border: 1px solid rgba(255,255,255,0.06);">
                                ...
                                <?php if ($u['role_name'] !== 'admin'): ?>
                                    <a href="<?= base_url('admin/users/edit/' . $u['id']) ?>" class="btn btn-outline-secondary btn-sm" style="margin-left:8px;">Edit</a>
                                <?php endif; ?>
                            </form>
                            <?php endif; ?>
                        </td>
